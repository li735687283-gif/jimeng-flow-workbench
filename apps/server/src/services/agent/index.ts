...
你必须严格返回一个 JSON 对象（不要包含任何 markdown 代码块标记、不要任何解释性文字），结构如下：
{
  ...
  "thinking": "详细的思考过程：先分析用户意图（生成图片、生成视频、还是纯文本对话），然后分析画面元素、风格、镜头等，最后给出优化策略（3-5 句话，可折叠展示给用户）",
  "intent": "image 或 video 或 text 或 image_then_video",
...
1. intent 字段必须严格为以下之一：
   ...
   - "video"：用户明确要求生成视频（如"生成视频"、"做一段视频"）
   - "text"：用户只是纯文本对话，不需要生成内容
   - "image_then_video"：用户可能先生成图片，再基于图片生成视频（如"做一个视频海报"）
2. thinking 字段要详细展示你的分析过程，包括意图识别、画面拆解、优化策略。
3. optimizedPrompt 要具体、可执行、细节丰富，包含主体、场景、风格、镜头、光线、材质、构图等。
4. negativePrompt 列出常见需要避免的内容。
5. suggestedParams 给出合理的尺寸和数量建议（width/height 用 1024 或 1280，count 用 1-4）。
6. proposedActions 至少给一个建议动作，例如 create_prompt_node（创建 Prompt 节点）或 overwrite_prompt（覆盖当前 Prompt）。
...
function parseIntent(v: unknown): PromptOptimizeResponse['intent'] {
  if (v === 'image' || v === 'video' || v === 'text' || v === 'image_then_video') return v
  return undefined
}
