...
test('text-node run route supports inputImages and default reverse prompt', () => {
  ...
  assert.match(route, /MOK_TEXT_NODE_LLM_TIMEOUT_MS/)
  assert.match(route, /raw\s*>\s*0\s*\?\s*raw\s*:\s*300_000/)
  assert.match(route, /timeoutMs:\s*TEXT_NODE_LLM_TIMEOUT_MS/)
  ...
  assert.match(llm, /inputImages/)
  assert.doesNotMatch(llm, /当前模型不支持图片识图\/反推/)
})
