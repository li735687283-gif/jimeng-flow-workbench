...
应用只在正式打包环境启动后检查更新。开发模式不会访问真实更新源；也可用 `MOK_DISABLE_AUTO_UPDATE=1` 临时禁用。没有更新时不显示提醒。发现更新时，首页左上角头像会出现红色下载标志，悬停显示“有新版本，是否下载？”，但不会自动下载；点击后标志切换为圆形下载进度，下载完成时自动关闭并重启 MO.K 完成安装。忽略提醒不会下载，应用会在下次启动时重新检查。网络、版本解析或下载失败只记录错误，不会让应用崩溃，下载失败后可再次点击重试。
