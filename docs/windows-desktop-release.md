...
应用只在正式打包环境启动后检查更新。开发模式不会访问真实更新源；也可用 `MOK_DISABLE_AUTO_UPDATE=1` 临时禁用。没有更新时不弹窗。发现更新时会先提示用户；选择“下载并安装”后才开始下载，下载完成时自动关闭并重启 MO.K 完成安装。选择“暂不更新”不会下载，应用会在下次启动时重新检查。网络、版本解析或下载失败只记录错误，不会让应用崩溃。
